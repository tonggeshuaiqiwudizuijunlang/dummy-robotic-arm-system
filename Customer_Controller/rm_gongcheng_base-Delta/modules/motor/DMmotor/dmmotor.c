...
void DMMotorEnable_test(DMMotorInstance *motor)
{
    for(int i=0; i<7; i++)
    {
        motor->motor_can_instance->tx_buff[i] = 0xFF;
    }
    motor->motor_can_instance->tx_buff[7] = 0xFC;
...
void DMMotorSetMITRef(DMMotorInstance *motor, float MIT_position,float MIT_velocity,float MIT_KP,float MIT_KD,float MIT_torque)
{
    motor->MIT_position = MIT_position;
    motor->MIT_velocity = MIT_velocity;
    motor->MIT_KP = MIT_KP;
    motor->MIT_KD = MIT_KD;
    motor->MIT_torque = MIT_torque;
}
...
void DMMotorTask(void const *argument)
{
   ...
   //DM_Motor_Measure_s *measure = &motor->measure;
    ...
    while (1)
    {   
        set = motor->MIT_position;
        if (setting->motor_reverse_flag == MOTOR_DIRECTION_REVERSE)
            set *= -1;

        LIMIT_MIN_MAX(set, DM_P_MIN, DM_P_MAX);//位置限幅
        motor_send_mailbox.position_des = float_to_uint(set, DM_P_MIN, DM_P_MAX, 16);
        motor_send_mailbox.velocity_des = float_to_uint(motor->MIT_velocity, DM_V_MIN, DM_V_MAX, 12);
        motor_send_mailbox.torque_des = float_to_uint(motor->MIT_torque, DM_T_MIN, DM_T_MAX, 12);
        motor_send_mailbox.Kp = float_to_uint(motor->MIT_KP, DM_KP_MIN, DM_KP_MAX, 12);
        motor_send_mailbox.Kd = float_to_uint(motor->MIT_KD, DM_KD_MIN, DM_KD_MAX, 12);
