...
// void StartCailTASK(void const *argument);
...
void OSTaskInit()
{
    ...
    // osThreadDef(calitask, StartCailTASK,  osPriorityNormal, 0, 1024);
    // cailTaskHandle = osThreadCreate(osThread(calitask), NULL);
...
// void StartCailTASK(void const *argument)
// {    
//     static float cail_start;
//     static float cail_dt;
//     static uint8_t test_flag=0;
//     // 执行校准（带重试机制）
//     for(;;) 
//     {

//         cail_start = DWT_GetTimeline_ms();

//         cail_dt = DWT_GetTimeline_ms() - cail_start;
//         if (cail_dt > 1)
//             LOGERROR("[freeRTOS] INS Task is being DELAY! dt = [%f]", &ins_dt);
...
//     // 校准失败处理
//     LOGERROR("Calibration failed after 3 attempts");
//     vTaskDelete(NULL);
// }
